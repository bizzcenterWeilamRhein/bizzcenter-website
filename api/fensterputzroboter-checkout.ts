...
const TARIFE: Record<string, { label: string; preisTag: number }> = {
  'fensterputzroboter': { label: 'Fensterputzroboter mieten (ECOVACS WINBOT W2S Omni)', preisTag: 3900 },
};
...
const STAFFEL: Record<string, { min: number; max: number; preis: number }[]> = {
  'fensterputzroboter': [
    ...
    { min: 2, max: 2, preis: 7900 },
    { min: 3, max: 3, preis: 10900 },
    { min: 4, max: 7, preis: 15900 },
  ],
};

const KAUTION_CENTS = 20000; // EUR 200,00

function getStaffelPreis(tarif: string, tage: number): number {
  const staffel = STAFFEL[tarif] || STAFFEL['fensterputzroboter'];
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const overlap = await pool.query(
      `SELECT id FROM fensterputzroboter_bookings
    ...
    if (overlap.rows.length > 0) {
      return res.status(409).json({ error: 'Der Fensterputzroboter ist in diesem Zeitraum bereits gebucht.', code: 'NOT_AVAILABLE' });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        ...
        },
        {
          price_data: {
            currency: 'eur',
            unit_amount: KAUTION_CENTS,
            product_data: {
              name: 'Kaution (wird nach Rückgabe erstattet)',
              description: 'Kaution EUR 200,00 — wird nach unbeschädigter Rückgabe vollständig erstattet.',
            },
          },
          quantity: 1,
        },
      ...
      success_url: `${SITE_URL}/fensterputzroboter-mieten?buchung=erfolgreich&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${SITE_URL}/fensterputzroboter-mieten?buchung=abgebrochen`,
      tax_id_collection: { enabled: true },
    });

    // Save booking as confirmed (payment pending)
    await pool.query(
      `INSERT INTO fensterputzroboter_bookings (start_date, end_date, tarif, tage, gesamtpreis_netto, vorname, nachname, firma, email, telefon, stripe_session_id, stripe_payment_status, status, ausweis_uploaded)
    ...
    console.error('Fensterputzroboter checkout error:', err);
