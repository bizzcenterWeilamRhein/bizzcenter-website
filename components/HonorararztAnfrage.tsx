...
interface HonorararztAnfrageProps {
  title?: string;
  description?: string;
  /** 'full' = volle Section mit eigener H2 + Container; 'compact' = nur das Formular (für Einbau in den Hero) */
...
export function HonorararztAnfrage({
  ...
  // ─── Success-State (gleich für beide Varianten) ─────────────────
  if (sent) {
    const successCard = (
  ...
  // ─── Form-Inhalt (gleich für beide Varianten) ───────────────────
  const formInner = (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7f3e] focus:border-[#6b7f3e] outline-none text-sm bg-white"
          >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            required
            minLength={2}
            className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7f3e] focus:border-[#6b7f3e] outline-none text-sm"
            placeholder="Vorname"
          />
        </div>
        <div>
      ...
      <div>
        ...
        <PhoneInput
          ...
          inputClassName="flex-1 min-w-0 px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7f3e] focus:border-[#6b7f3e] outline-none text-sm"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Fachrichtung</label>
          <select
            value={fachrichtung}
          ...
          >
            <option value="">Bitte wählen</option>
            {FACHRICHTUNGEN.map(f => <option key={f} value={f}>{f}</option>)}
          </select>
        </div>
        <div>
      ...
      {/* Anmerkungen — in compact-Variante kompakter */}
      {variant === 'full' && (
        <div>
          ...
          <textarea
            ...
            rows={2}
            className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7f3e] focus:border-[#6b7f3e] outline-none resize-none text-sm"
      ...
      <button
        ...
        className="w-full py-3 px-6 bg-[#6b7f3e] text-white font-semibold rounded-lg hover:bg-[#5a6b34] disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm"
      >
  ...
  // ─── Compact-Variante: nur das Form (für Einbau im Hero) ────────
  if (variant === 'compact') {
    return (
  ...
  // ─── Full-Variante: Section + Container + H2 ─────────────────────
  return (
    <section className="py-16 px-4" id="formular">
