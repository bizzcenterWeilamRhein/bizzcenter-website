...
interface AnwaltAnfrageProps {
  title?: string;
  description?: string;
  variant?: 'full' | 'compact';
}

export function AnwaltAnfrage({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const firmaDisplay = [titelClean, anredeClean].filter(Boolean).join(' ').trim() || 'Rechtsanwalt';
    ...
    try {
      const res = await fetch('/api/lead', {
        ...
        body: JSON.stringify({
          ...
          quelle: 'rechtsanwalt-landingpage',
      ...
      trackLeadSubmitted('rechtsanwalt_landingpage', {
  ...
  const formInner = (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Fachgebiet</label>
          <select value={fachgebiet} onChange={e => setFachgebiet(e.target.value)} className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7f3e] focus:border-[#6b7f3e] outline-none text-sm bg-white">
            <option value="">Bitte wählen</option>
      ...
      {variant === 'full' && (
        <div>
          ...
          <textarea value={anmerkungen} onChange={e => setAnmerkungen(e.target.value)} rows={2} className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7f3e] focus:border-[#6b7f3e] outline-none resize-none text-sm" placeholder="z.B. RAK-Bezirk, Sonderwünsche, geplante Kanzleiform..." />
        </div>
      )}
