...
const TARIFE: Tarif[] = [
  {
    ...
    label: 'ECOVACS WINBOT W2S Omni',
    preis: 39,
    beschreibung: 'Fensterputzroboter Komplett-Set',
    details: ['ECOVACS WINBOT W2S Omni', 'Multifunktionsstation', 'Reinigungspads (2 Saetze)', 'Sicherheitsseil', 'Bedienungsanleitung'],
  },
];

// Staffelpreise (Fensterputzroboter)
...
const STAFFEL_ROBOTER: StaffelPreis[] = [
  { minTage: 1, maxTage: 1, label: '1 Tag', gesamtpreis: 39 },
  { minTage: 2, maxTage: 2, label: '2 Tage', gesamtpreis: 79 },
  { minTage: 3, maxTage: 3, label: 'Wochenende (Fr-So)', gesamtpreis: 109 },
  { minTage: 4, maxTage: 7, label: 'Woche', gesamtpreis: 159 },
];

function getStaffelPreis(tarif: string, tage: number): number {
  const staffel = STAFFEL_ROBOTER;
  // Find matching tier
...
export function FensterputzroboterBuchung() {
  const [selectedTarif, setSelectedTarif] = useState<string>('fensterputzroboter');

  // Only one tarif now
  useEffect(() => {
    setSelectedTarif('fensterputzroboter');
  }, []);
  ...
  const fetchAvailability = useCallback(async (month: number, year: number) => {
    ...
    try {
      const res = await fetch(`/api/fensterputzroboter-availability?month=${month + 1}&year=${year}`);
      if (res.ok) {
  ...
  const handleDayClick = (day: number) => {
    ...
    if (!selectingEnd || !startDate) {
      ...
      if (hasBookedInRange(startDate, ds)) {
        setError('Im gewaehlten Zeitraum ist der Fensterputzroboter bereits gebucht. Bitte waehlen Sie einen anderen Zeitraum.');
        return;
      }
      setEndDate(ds);
      setSelectingEnd(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/fensterputzroboter-checkout', {
        method: 'POST',
    ...
    return (
      <section className="max-w-3xl mx-auto px-4 py-12">
        <div className="bg-[#f0f4e8] border border-[#6b7f3e]/20 rounded-2xl p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">Vielen Dank! Ihre Fensterputzroboter-Buchung ist bestätigt und bezahlt.</p>
          ...
          <p className="text-sm text-gray-500 mt-4">Bitte bringen Sie zur Abholung EUR 200,- Kaution (bar oder Karte) mit.</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="max-w-4xl mx-auto px-4 py-12">
      <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Fensterputzroboter buchen</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            ...
            <div className="p-5 rounded-xl border-2 border-[#6b7f3e] bg-[#f0f4e8]">
              <div className="flex justify-between items-start mb-2">
                <span className="font-bold text-gray-900">ECOVACS WINBOT W2S Omni</span>
                <span className="text-[#6b7f3e] font-bold text-lg">ab EUR 39,-</span>
              </div>
              <p className="text-sm text-gray-500 mb-3">Fensterputzroboter Komplett-Set</p>
              <ul className="space-y-1">
                <li className="text-xs text-gray-600 flex items-center gap-1.5"><span className="text-[#6b7f3e]">&#10003;</span> ECOVACS WINBOT W2S Omni</li>
                <li className="text-xs text-gray-600 flex items-center gap-1.5"><span className="text-[#6b7f3e]">&#10003;</span> Multifunktionsstation</li>
                <li className="text-xs text-gray-600 flex items-center gap-1.5"><span className="text-[#6b7f3e]">&#10003;</span> Reinigungspads (2 Sätze)</li>
                <li className="text-xs text-gray-600 flex items-center gap-1.5"><span className="text-[#6b7f3e]">&#10003;</span> Sicherheitsseil</li>
                <li className="text-xs text-gray-600 flex items-center gap-1.5"><span className="text-[#6b7f3e]">&#10003;</span> Bedienungsanleitung</li>
              </ul>
          ...
          <div>
            ...
            <div className="mt-3 border border-gray-200 rounded-lg overflow-hidden">
              <div className="bg-gray-50 px-4 py-2 border-b border-gray-200">
                <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Mehrtages-Rabatt (Fensterputzroboter)</span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-gray-200">
                {STAFFEL_ROBOTER.map((s, i) => (
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <input type="checkbox" id="roboter-privat" checked={isPrivat} onChange={e => { setIsPrivat(e.target.checked); if (e.target.checked) setFirma(''); }}
                  className="w-4 h-4 text-[#6b7f3e] border-gray-300 rounded focus:ring-[#6b7f3e]" />
                <label htmlFor="roboter-privat" className="text-sm text-gray-700">Ich buche als Privatperson (keine Firma)</label>
              </div>
          ...
          <div className="bg-[#f5f0eb] rounded-lg px-4 py-3 text-sm text-gray-600 space-y-1">
            ...
            <p>Bei Abholung: <strong>EUR 200,- Kaution</strong> (bar oder Karte). Kaution wird bei Rückgabe erstattet.</p>
            {!ausweisFile && (
